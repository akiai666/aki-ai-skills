...
> **Interflow AI 出品** — 这是面向公开复用的 Agent Skill。请不要提交私有素材、密钥或生成草稿。
...
- **16 种精选视觉风格**（11 款核心 + **炸裂族** 6 款竖屏吸睛）× 多种布局 × 3 种视频边框，自由组合；旗舰 **暗夜星河** `nebula-glass` 是黑底双星粒子场 + 磨砂玻璃 + 归藏排版
- **横屏与竖屏走两条审美**：横屏分享走**简洁克制的归藏排版风**（暗夜星河 / 玻璃拟态这类冷调玻璃 + 强字阶 + 大留白），信息清楚、高级、耐看；竖屏社媒尽量**吸睛炸裂**，优先**炸裂族**（霓虹巨字、液态极光、电影光晕，3 秒抓人）
...
1. 把本仓库 clone 到本地，或直接使用当前仓库里的 skill 目录：

   ```bash
   git clone https://github.com/akiai666/aki-ai-skills.git
   cd aki-ai-skills
   ```

2. 同步到本机 Agent skills 目录：

   ```bash
   bash scripts/sync_to_agents.sh interflow-video-cut
   ```

   默认目标是 `~/.agents/skills/interflow-video-cut`。也可以用 `AGENT_SKILLS_DIR=/path/to/skills` 覆盖。
3. 确保系统已装依赖（见下）。
4. 在对话里说「用 interflow-video-cut 剪这条视频 /path/to/video.mp4」即可。
...
> - **横屏**（YouTube / B站 / 桌面播放 / 长内容）—— 走**简洁克制的归藏排版风**：**暗夜星河** `nebula-glass`、**玻璃拟态** `glass` 这条冷调玻璃 + 强字阶 + 大留白的路线，信息清楚、高级、耐看，横屏大画面里最稳。
