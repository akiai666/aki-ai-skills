  ...
  questions: [
    ...
    },
    {
      ...
      options: [
        ...
        { label: "Aki 清透 HUD (by-aki-clean-hud)", description: "基于暖玻 HUD 二开：保留 HUD 文字动画和局部磨砂，但不加全屏遮罩、不压暗原视频。适合想保留原镜头质感的口播解释。" },
...
3) 视觉风格 — 对照桌面总览图 (~/Desktop/interflow-style-gallery/) 报具体风格 (中文名或 key)：
   ...
   4. Aki 清透 HUD by-aki-clean-hud — 暖玻 HUD 二开，不加全屏遮罩、不压暗原视频
   5. 暖光太空 spatial     — 黑底暖橙光，温暖有空间感
   6. 撞色大字 geom        — 黑底亮色块超大字，大胆冲击
   ── 干净专业（数据·报告·严肃）──
   7. 瑞士网格 swiss       — 白底红点大字，专业权威
   8. 黑白极简 minimal     — 纯黑白大字+大留白，克制高级
   9. 代码终端 terminal    — 黑底绿字代码风，技术极客
   ── 浅色 / 杂志 ──
   10. 柔光浅色 pastel-aura — 浅色柔和，白天日常
   11. 杂志印刷 editorial-print — 照片排成杂志跨页（非文字卡）
...
2. **Map the style group to a specific style** by looking at the
   ...
   **2·风格家族 — 主风格锁定，重点卡可配姊妹风格（治「整片一个模子」）.**
   ...
   - **只在同一家族内挑姊妹**（沿用 skill 已有的家族划分：**炸裂族**
     ...
     by-aki-clean-hud / spatial / nebula-glass；**clinical** swiss / minimal / terminal；外加
...
(pastel-aura / glass / glass-hud / by-aki-clean-hud / spatial / nebula-glass / minimal / geom / terminal /
...
|---|---|---|
| **style** | `minimal` `geom` `terminal` `swiss` `pastel-aura` `glass` `glass-hud` `by-aki-clean-hud` `spatial` `nebula-glass` `editorial-print` | the card's visual language — fonts, colors, ornament, layout-within-card |
